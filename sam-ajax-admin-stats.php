  ...
  switch($action) {
    case 'sam_ajax_load_stats':
      if(isset($_POST['id'])) {
        ...
        $now = getdate();
        $month = $now['mon'] - $sMonth;
        $year = $now['year'];
        $days = samDaysInMonth($month, $year);
        $sql = "SELECT
                  DATE_FORMAT(ss.event_time, %s) AS ed,
                  COUNT(*) AS hits
                FROM $sTable ss
                WHERE (EXTRACT(YEAR_MONTH FROM NOW()) - %d = EXTRACT(YEAR_MONTH FROM ss.event_time)) AND ss.event_type = %d AND ss.pid = %d
                GROUP BY ed;";
        $hits = $wpdb->get_results($wpdb->prepare($sql, '%e', $sMonth, 0, $pid), ARRAY_A);
        $clicks = $wpdb->get_results($wpdb->prepare($sql, '%e', $sMonth, 1, $pid), ARRAY_A);
        for($i = 1; $i <= $days; $i++) {
        ...
        $sql = "SELECT
                  (SELECT
                    COUNT(*)
                    FROM $sTable ss
                    WHERE (EXTRACT(YEAR_MONTH FROM NOW()) - %d = EXTRACT(YEAR_MONTH FROM ss.event_time))
        ...
        $total = $wpdb->get_row($wpdb->prepare($sql, $sMonth, $pid, $sMonth, $pid));
    ...
    case 'sam_ajax_load_item_stats':
      if(isset($_POST['id'])) {
        ...
        $sql = "SELECT
                  ...
                  COUNT(*) AS hits
                FROM $sTable ss
                WHERE (EXTRACT(YEAR_MONTH FROM NOW()) - %d = EXTRACT(YEAR_MONTH FROM ss.event_time)) AND ss.event_type = %d AND ss.id = %d
                GROUP BY ed;";
        $hits = $wpdb->get_results($wpdb->prepare($sql, '%e', $sMonth, 0, $id), ARRAY_A);
        $clicks = $wpdb->get_results($wpdb->prepare($sql, '%e', $sMonth, 1, $id), ARRAY_A);
        for($i = 1; $i <= $days; $i++) {
          $hitsFull[$i - 1] = array( $i, 0);
          $clicksFull[$i - 1] = array($i, 0);
        }
        ...
        $total = $wpdb->get_row($wpdb->prepare($sql, $sMonth, $id, $sMonth, $id));
    ...
    case 'sam_ajax_load_ads':
      if(isset($_POST['id'])) {
        ...
        $sql = "SELECT
                  ...
                  sa.name,
                  sa.description,
                  @ad_hits := (SELECT COUNT(*) FROM $sTable ss WHERE (EXTRACT(YEAR_MONTH FROM NOW()) - %d = EXTRACT(YEAR_MONTH FROM ss.event_time)) AND ss.id = sa.id AND ss.pid = sa.pid AND ss.event_type = 0) AS ad_hits,
                  @ad_clicks := (SELECT COUNT(*) FROM $sTable ss WHERE (EXTRACT(YEAR_MONTH FROM NOW()) - %d = EXTRACT(YEAR_MONTH FROM ss.event_time)) AND ss.id = sa.id AND ss.pid = sa.pid AND ss.event_type = 1) AS ad_clicks,
                  sa.ad_weight,
        ...
        $rows = $wpdb->get_results($wpdb->prepare($sql, $sMonth, $sMonth, $id), ARRAY_A);
