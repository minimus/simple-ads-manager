...
w2utils.event = {
    ...
    on: function (eventData, handler) {
        if (!jQuery.isPlainObject(eventData)) eventData = { type: eventData };
        eventData = jQuery.extend({ type: null, execute: 'before', target: null, onComplete: null }, eventData);
        ...
        if (!jQuery.isArray(this.handlers)) this.handlers = [];
    ...
    trigger: function (eventData) {
        var eventData = jQuery.extend({ type: null, phase: 'before', target: null }, eventData, {
        ...
        for (var h = this.handlers.length-1; h >= 0; h--) {
            ...
            {
                eventData = jQuery.extend({}, item.event, eventData);
...
w2utils.keyboard = (function (obj) {
    ...
    function keydown (event) {
        ...
        if (jQuery.inArray(tag, ['INPUT', 'SELECT', 'TEXTAREA']) !== -1) return;
        if (jQuery(event.target).prop('contenteditable') === 'true') return;
        if (!w2ui_name) return;
    ...
    function mousedown (event) {
        var tag = event.target.tagName;
        var obj = jQuery(event.target).parents('.w2ui-reset');
        if (obj.length > 0) {
                ...
                if (mitem.hidden !== true) {
                    ...
                    if (typeof txt !== 'undefined' && txt !== '' && !(/^-+$/.test(txt))) {
                        ...
                        menu_html +=
                            ...
                            '               jQuery(\'#w2ui-overlay'+ name +'\').remove(); '+
                            '               jQuery.fn.w2menuHandler(event, \''+ f +'\');">'+
                                imgd +
    ...
    w2grid.prototype = {
        ...
        refresh: function () {
            ...
            bodyHTML +=  '<div id="grid_'+ this.name +'_records" class="w2ui-grid-records"'+
                        ...
                        '        jQuery(\'#grid_'+ this.name +'_columns\')[0].scrollLeft = this.scrollLeft;'+
                        '        jQuery(\'#grid_'+ this.name +'_summary\')[0].scrollLeft = this.scrollLeft;'+
                        '        obj.scroll(event);">'+
                            this.getRecordsHTML() +
                        '</div>'+
        ...
        initColumnOnOff: function () {
            ...
            var col_html =  '<div class="w2ui-col-on-off">'+
                            ...
                            '</td>'+
                            '<td onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'line-numbers\'); jQuery(\'#w2ui-overlay\')[0].hide();">'+
            ...
            col_html += '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateSave(); jQuery(\'#w2ui-overlay\')[0].hide();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Save Grid State') + '</div>'+
                        '</td></tr>'+
                        '<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].stateReset(); jQuery(\'#w2ui-overlay\')[0].hide();">'+
...
var w2alert = function (msg, title, callBack) {
    ...
    if (jQuery('#w2ui-popup').length > 0 && w2popup.status != 'closing') {
        w2popup.message({
            width   : 400,
            height  : 170,
...
var w2confirm = function (msg, title, callBack) {
    ...
    var defaults = {
        ...
        width       : (jQuery('#w2ui-popup').length > 0 ? 400 : 450),
        height      : (jQuery('#w2ui-popup').length > 0 ? 170 : 220),
        yes_text    : 'Yes',
        yes_class   : '',
        yes_style   : '',
        yes_callBack: null,
    ...
    if (arguments.length == 1 && typeof msg == 'object') {
        jQuery.extend(options, defaults, msg);
    } else {
        ...
        if (options.height > (w2popup.get().height - 50)) options.height = w2popup.get().height - 50;
          w2popup.message({
            ...
            html    : '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' +
                      ...
                      '</div>',
            onOpen: function () {
                jQuery('#w2ui-popup .w2ui-popup-message .btn').on('click', function (event) {
                    w2popup.message();
        ...
        w2popup.open({
            ...
            onOpen: function (event) {
                event.onComplete = function () {
                    jQuery('#w2ui-popup .w2ui-popup-btn').on('click', function (event) {
                        w2popup.close();
                        if (typeof options.callBack == 'function') options.callBack(event.target.id);
                        if (event.target.id == 'Yes' && typeof options.yes_callBack == 'function') options.yes_callBack();
                        if (event.target.id == 'No'  && typeof options.no_callBack == 'function') options.no_callBack();
            ...
            switch (item.type) {
                ...
                case 'drop':
                    ...
                    html += '<table cellpadding="0" cellspacing="0" title="'+ item.hint +'" class="w2ui-button '+ (item.checked ? 'checked' : '') +'" '+
                            ...
                            '       onmouseover = "' + (!item.disabled ? "jQuery(this).addClass('over');" : "") + '"'+
                            '       onmouseout  = "' + (!item.disabled ? "jQuery(this).removeClass('over').removeClass('down');" : "") + '"'+
